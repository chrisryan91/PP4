...
class AdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.cuisine_type = \
        CuisineType.objects.create(name='Test Cuisine Type')
    ...
    def test_review_admin(self):
        ...
        self.assertEqual(
            ...
            ('title', 'recipe', 'slug', 'status', 'created_on'))

        self.assertEqual(
            post_admin.search_fields,
            ['title', 'recipe', 'content', 'email'])

        self.assertEqual(post_admin.summernote_fields, ("content"))

        self.assertEqual(post_admin.actions, ['approve_reviews'])

        post_admin.approve_reviews(
            request=request,
            queryset=Review.objects.filter(pk=self.review.pk))
    ...
    def test_comment_admin(self):
        ...
        self.assertEqual(
            ...
            ('name', 'body', 'created_on', 'approved', 'review'))

        self.assertEqual(
            comment_admin.list_filter,
            ('approved', 'created_on', 'review'))
        ...
        comment_admin.approve_comments(
                request=request,
                queryset=Comment.objects.filter(pk=self.comment.pk))
